class FoldsController {
  ...
  /*numberFolds(array dataSet) --> int
    Donne le nombre de folds dont on aura besoin pour le dataset.*/
  numberFolds(dataSet) {
    let numberOfFolds = 10;
    if (dataSet.length % 10 != 0) {
      //Pour éviter de laisser certains données sans les tester --> sur 158 données, 8 données ne pourraient pas être testé si on divise par 10 --> création d'un 11ème fold avec le reste
      numberOfFolds = 11;
    }
    this._numberOfFolds = numberOfFolds;
    return numberOfFolds;
  }
  /*createFolds(int numberOfFolds) --> none
    creation de dossiers pour repartir les differents donnees en 10/11 parties */
